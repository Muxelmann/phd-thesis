    ...
    \foreach[count=\i, evaluate=\i as \l using int(\i*2)] \n in {1,2,...,\N}
	{
    	...
    	\ifthenelse{\n=1}
    	{
    		...
    		\ifthenelse{\n<\N}
			{
				\ifthenelse{\dn=1}
				{
					\draw [->] (z_a\dn) to node[left]{$p(t-\tau)$} (z_a\n);
				}{
					\draw [->] (z_a\dn) to node[left]{$p(t-\dn\tau)$} (z_a\n);
				}
				\ifthenelse{\dnn=0}
				{
					...
					\ifthenelse{\dnn=1}
					{
						\draw [->] (z_b\dn) to node[right]{$\hat{p}(t-\tau)$} (z_b\n);
					}{
						\draw [->] (z_b\dn) to node[right]{$\hat{p}(t-\dnn\tau)$} (z_b\n);
					}
				}
				\draw [->] (adder_\n) to (adder_\dn);
			}{
...
\end{tikzpicture}
\caption{Exogenous auto-regressive model that is used for model predictive control. Here, $z^{-1}$ indicates the time delay by one sample period, i.e. $\tau$.}
