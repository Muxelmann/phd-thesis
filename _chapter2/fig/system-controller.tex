...
\begin{tikzpicture}[node distance=2cm, shorten >= 1pt, >=stealth', auto, scale=0.8, transform shape]
    ...
    node [box, fill=blue!20, below of=pid_soc, yshift=-10mm](schedule) {Schedule}
    node [box, fill=green!20, below of=pid_p, yshift=-10mm](mpc) {MPC};
	% Draw lines
	...
	\draw [->] (network.0) to [out=340, in=60] (pid_p.0|-pid_p.90);
	\draw [->] (schedule) to node[pos=0.25] {$SOC(t)$} (pid_soc);
	\draw [->] (mpc) to node[pos=0.25] {$\hat{p}_{net}(t+\Delta t)$} (pid_p);
	\draw [->, bend left] (network.0) to [out=80, in=100] node[right, pos=0.1, xshift=2mm, yshift=1mm] {$p_{net}(t)$} (mpc.0);
	
	\draw (-4, -6.25) node [right] {Controller};
	
	\draw [dashed] (-4, -3) -- (4, -3) -- (4, -6) -- (-4, -6) -- (-4, -3);
