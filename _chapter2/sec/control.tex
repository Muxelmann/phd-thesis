...
\nomenclature[J]{$p_\text{net}(t)$}{Most recent network demand at sample $t$, where $p_\text{net}(t) \in \textbf{p}_{net}$ (Chapter \ref{ch2})}
\nomenclature[J]{$SOC^*(t)$}{Battery's state of charge at sample $t$ (Chapter \ref{ch2})}
\nomenclature[J]{$E_\text{SOC}(t)$}{Error in state of charge at sample $t$ (Chapter \ref{ch2})}
\nomenclature[J]{$E_{p}(t)$}{Difference between current and predicting network power at sample $t$, where $p(t) \in \textbf{p}$ (Chapter \ref{ch2})}
\nomenclature[J]{$\hat{p}_\text{net}(t+\Delta t)$}{Predicted next network power at sample $t$ (Chapter \ref{ch2})}
\nomenclature[J]{$\alpha$}{PID weight vector for compensator PID$_1$ (Chapter \ref{ch2})}
\nomenclature[J]{$\beta$}{PID weight vector for compensator PID$_2$ (Chapter \ref{ch2})}
\nomenclature[J]{$a$}{Weight vector for compensator input regression of the AR model (Chapter \ref{ch2})}
\nomenclature[J]{$b$}{Weight vector for compensator output regression of the AR model (Chapter \ref{ch2})}
\nomenclature[J]{$N$}{Number of regressors of the AR model (Chapter \ref{ch2})}
\nomenclature[J]{$p_1(t)$, $p_2(t)$}{Corrective BESS power components from PID$_1$ and PID$_2$, respectively (Chapter \ref{ch2})}
\nomenclature[J]{$SOC_\text{tol}$}{SOC tolerance, i.e. maximum deviation from the prescheduled SOC profile (Chapter \ref{ch2})}
