...
This section explains the dynamic control (i.e. the controller block as shown in Figure~\ref{ch2:subfig:proposed-dynamic-control-system}) \hladd{in the shape of an MPC, }\hlrem{which contains}\hladd{containing} the two PID compensators to adjust operation around the predetermined ESMU schedule.
The first PID compensator is fed by the ESMU schedule and the other is fed by the \hlrem{MPC}\hladd{predictor} load estimations.
...
\nomenclature[J]{$\boldsymbol{\beta}$}{PID weight vector for \hlrem{MPC}\hladd{predictor} compensator PID$_2$, where $\boldsymbol{\beta} = \{\beta_P, \beta_I, \beta_D\}$ and $\boldsymbol{\beta} \in \mathbb{R}^3$}
...
This value is the PID$_2$'s input from the \hlrem{MPC}\hladd{predictor} and results in an ``estimated power error signal'':
...
\hladd{The system of two PID compensators for discrete time is unconventional and it may be worth considering different types of control or compensator.
However, with the above-explained approach and for the data used as part of this research, the computed}\hlrem{
This resulting} set of $\boldsymbol{\alpha}$ and $\boldsymbol{\beta}$ weights \hlrem{therefore guaranteed}\hladd{resulted} a convergent and stable solutions\hlrem{ for the provided data}.
\hladd{In this context, convergent means that the $SOC^*(t)$ values tend towards the $SOC(t)$ values, and stable means that the $SOC^*(t)$ values never clipped at zero or one.}
