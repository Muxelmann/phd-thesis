...
\nomenclature{$p_{net}(t)$}{Most recent network demand at sample $t$, where $p_{net}(t) \in \textbf{p}_{net}$ (Chapter \ref{ch2})}
\nomenclature{$SOC^*(t)$}{Battery's state of charge at sample $t$ (Chapter \ref{ch2})}
\nomenclature{$E_{SOC}(t)$}{Error in state of charge at sample $t$ (Chapter \ref{ch2})}
...
\nomenclature{$\hat{p}_{net}(t+\Delta t)$}{Predicted next network power at sample $t$ (Chapter \ref{ch2})}
...
\nomenclature{$SOC_{tol}$}{SOC tolerance, i.e. maximum deviation from the prescheduled SOC profile (Chapter \ref{ch2})}
