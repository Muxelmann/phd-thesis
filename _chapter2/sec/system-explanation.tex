...
\nomenclature[J]{$P_{bat}$}{BESS power electronic rating, where $P_{bat} \in \mathbb{Z}^{>0}$}
...
\nomenclature[J]{$p(t)$}{BESS power at time $t$, where $(p(t)) = \textbf{p}$ and $p(t) \in \mathbb{Z}$}
\nomenclature[J]{$\textbf{p}$}{BESS power vector, where $(p(t)) = \textbf{p}$}
\nomenclature[J]{$p_{bat}(t)$}{Battery power at time $t$, which is derived form $p(t)$, where $(p_{bat}(t)) = \textbf{p}_{bat}$ and $p(t) \in \mathbb{Z}$}
\nomenclature[J]{$\textbf{p}_{bat}$}{Battery power vector, where $(p_{bat}(t)) = \textbf{p}_{bat}$}
...
\nomenclature[J]{$p_\text{for}(k(t))$}{Half-hourly load forecast that is used for computing the BESS schedule, where $p_\text{for}(k(t))\in\mathbb{Z}$}
\nomenclature[J]{$\textbf{p}_\text{for}$}{Half-hourly load forecast vector that is used for computing the BESS schedule, where $(p_\text{for}(k(t))) = \textbf{p}_\text{for}$}
...
\nomenclature[J]{$\textbf{p}_\text{sch}$}{Half-hourly schedule vector that is generated from the load forecast, where $(p_\text{sch}(k(t))) = \textbf{p}_\text{sch}$}
