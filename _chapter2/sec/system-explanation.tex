...
\nomenclature{$C_\text{bat}$}{Battery capacity in kWh, where $C_\text{bat} \in \mathbb{Z}^{>0}$ (Chapter \ref{ch2})}
...
\nomenclature{$P_\text{bat}$}{BESS power electronic rating, where $P_\text{bat} \in \mathbb{Z}^{>0}$ (Chapter \ref{ch2})}
...
\nomenclature{$p_\text{bat}(t)$}{Battery power at time $t$, which is derived form $p(t)$, where $p_\text{bat}(t) \in \textbf{p}_\text{bat}$ and $p(t) \in \mathbb{Z}$ (Chapter \ref{ch2})}
...
\nomenclature{$p_\text{for}(k(t))$}{Half-hourly load forecast that is used for computing the BESS schedule, where $p_\text{for}(k(t))\in\textbf{p}_{for}$ and $p_\text{for}(k(t))\in\mathbb{Z}$ (Chapter \ref{ch2})}
\nomenclature{$p_\text{sch}(k(t))$}{Half-hourly schedule that is generated from the load forecast, where $p_\text{sch}(k(t))\in\textbf{p}_{sch}$ and$p_\text{sch}(k(t))\in\mathbb{Z}$ (Chapter \ref{ch2})}
...
\nomenclature{$\zeta_\text{PAR}(\textbf{p})$}{Cost of the underlying power profile, based on Peak-to-Average Ratio (PAR) (Chapter \ref{ch2})}
...
\nomenclature{$\zeta_\text{MM}(\textbf{p})$}{Cost of the underlying power profile, based on the difference between minimum and maximum power (Chapter \ref{ch2})}
...
\nomenclature{$\zeta_\text{TRA}(\textbf{p})$}{Cost of the underlying power profile, based on largest power transient (Chapter \ref{ch2})}
