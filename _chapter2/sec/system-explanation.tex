...
Therefore the same synchronisation function, $k(t)$,\hladd{ that was used in Section~}\ref{ch1:subsec:esmu-scheduling} can be used to link the native \hladd{minutely }sampling period of $\Delta t$ to the schedule's half-hourly \hlrem{period}\hladd{time slots}.
...
For simplicity linear forwarding was chosen which means that the power assigned at \hlrem{e.g.}\hladd{for instance} $t=1$ remains constant over the scheduling period of $30\Delta t$ until $t=31$.
...
The task at hand is to find a half-hourly ESMU schedule, $\textbf{p}_\text{sch}$, where $\textbf{p}_\text{sch} = (p_\text{sch}(k(t)))$, that improves the shape of the underlying forecast, \hlrem{e.g.}\hladd{for example} by reducing load peaks.
