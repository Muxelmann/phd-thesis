...
\nomenclature[J]{$p(t)$}{ESMU power at time $t$, where $(p(t)) = \textbf{p}$ and $p(t) \in \mathbb{Z}$}
\nomenclature[J]{$\textbf{p}$}{ESMU power vector, where $(p(t)) = \textbf{p}$}
...
\nomenclature[J]{$\textbf{p}_\text{for}$}{Half-hourly load forecast vector that is used for computing the ESMU schedule, where $(p_\text{for}(k(t))) = \textbf{p}_\text{for}$}
