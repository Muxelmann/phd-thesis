...
\nomenclature[J]{$C_\text{bat}$}{Battery capacity in kWh, where $C_\text{bat} \in \mathbb{Z}^{>0}$ (Chapter \ref{ch2})}
\nomenclature[J]{$C_{f}$}{Charge factor or ``C-factor'' of the battery, where $C_{f} \in \mathbb{Z}^{>0}$ (Chapter \ref{ch2})}
\nomenclature[J]{$\mu$}{Self-discharge losses of battery, where $\mu \in (0, 1]$ (Chapter \ref{ch2})}
\nomenclature[J]{$P_\text{bat}$}{BESS power electronic rating, where $P_\text{bat} \in \mathbb{Z}^{>0}$ (Chapter \ref{ch2})}
\nomenclature[J]{$\eta$}{Round-trip efficiency of power electronics, where $\eta \in (0, 1]$ (Chapter \ref{ch2})}
\nomenclature[J]{$t$}{Discrete sample of time, where $t \in [0, \Delta t, \dots, T\Delta t]$ (Chapter \ref{ch2})}
\nomenclature[J]{$T$}{Number of samples during the entire simulation (Chapter \ref{ch2})}
\nomenclature[J]{$\Delta t$}{High resolution sample period, i.e. sub-half hourly or minutely, where $\Delta t \in \mathbb{Z}^{>0}$ (Chapter \ref{ch2})}
\nomenclature[J]{$SOC(t)$}{Scheduled state of charge at sample $t$ (Chapter \ref{ch2})}
\nomenclature[J]{$p(t)$}{BESS power at time $t$, where $p(t) \in \textbf{p}$ and $p(t) \in \mathbb{Z}$ (Chapter \ref{ch2})}
\nomenclature[J]{$p_\text{bat}(t)$}{Battery power at time $t$, which is derived form $p(t)$, where $p_\text{bat}(t) \in \textbf{p}_\text{bat}$ and $p(t) \in \mathbb{Z}$ (Chapter \ref{ch2})}
...
\nomenclature[J]{$k(t)$}{Sampling time conversion function, linking sub-half hourly samples $t$ at sampling period $\Delta r$ to half-hourly period $30 \Delta t$ (Chapter \ref{ch2})}
...
\nomenclature[J]{$p_\text{for}(k(t))$}{Half-hourly load forecast that is used for computing the BESS schedule, where $p_\text{for}(k(t))\in\textbf{p}_{for}$ and $p_\text{for}(k(t))\in\mathbb{Z}$ (Chapter \ref{ch2})}
\nomenclature[J]{$p_\text{sch}(k(t))$}{Half-hourly schedule that is generated from the load forecast, where $p_\text{sch}(k(t))\in\textbf{p}_{sch}$ and$p_\text{sch}(k(t))\in\mathbb{Z}$ (Chapter \ref{ch2})}
...
\nomenclature[J]{$\zeta_\text{PAR}(\textbf{p})$}{Cost of the underlying power profile, based on Peak-to-Average Ratio (PAR) (Chapter \ref{ch2})}
...
\nomenclature[J]{$\zeta_\text{MM}(\textbf{p})$}{Cost of the underlying power profile, based on the difference between minimum and maximum power (Chapter \ref{ch2})}
...
\nomenclature[J]{$\zeta_\text{TRA}(\textbf{p})$}{Cost of the underlying power profile, based on largest power transient (Chapter \ref{ch2})}
...
\nomenclature[J]{$\zeta(\textbf{p})$}{Global cost for a given power profile $\textbf{p}$ (Chapter \ref{ch2})}
