...
\nomenclature[I]{$v_{\text{ESMU},\phi}(t)$}{Phase voltage at ESMU for phase $\phi$ at time $t$, where $(v_{\text{ESMU},\phi}(t)) = \textbf{v}_{ESMU}(t)$ and $v_{\text{ESMU},\phi}(t) \in \mathbb{C}$ (Chapter \ref{ch1})}
\nomenclature[I]{$\textbf{v}_\text{ESMU}(t)$}{Multi-phase voltage vector at ESMU at time $t$, where $(v_{\text{ESMU},\phi}(t)) = \textbf{v}_{ESMU}(t)$ and $v_{\text{ESMU},\phi}(t) \in \mathbb{C}^{\Phi}$ (Chapter \ref{ch1})}
...
\nomenclature[I]{$v_{\text{load},i,\phi}(t)$}{Phase voltage of load $i$ for phase $\phi$ at time $t$, where $(v_{\text{load},i,\phi}(t)) = \textbf{v}_\text{load}(t)$ and $v_{\text{load},i,\phi}(t) \in \mathbb{C}$ (Chapter \ref{ch1})}
\nomenclature[I]{$\textbf{v}_\text{load}(t)$}{Multi-phase load voltage vector at time $t$, where $(v_{\text{load},i,\phi}(t)) = \textbf{v}_\text{load}(t)$ and $\textbf{v}_\text{load}(t) \in \mathbb{C}^{I\times\Phi}$ (Chapter \ref{ch1})}
...
Therefore, the customer (or load) voltage is defined as $\textbf{v}_\text{load}(t)$, where $(v_{\text{load},i,\phi}(t)) = \textbf{v}_{load}(t)$, and used in the new cost function, $\zeta_\text{load voltage}(\textbf{v}_\text{load}(t))$, which is defined as:
...
Substituting the three-phase substation power vector, $\textbf{s}_{ss}(t)$ (where $(s_{\text{ss},\phi}) = \textbf{s}_{ss}$), for $\textbf{x}$, allows the formulation of another cost, i.e. the ``unbalance'' cost, $\zeta_\text{unbalance}(\textbf{s}_{ss}(t))$, which is defined as:
...
This equates to 12\% of the total network power ($\text{i.e. }\frac{s_\text{losses}(t)}{\sum_{\phi=1}^\Phi{s_{\text{ss},\phi}(t)}} \approx \frac{58kW}{484kW}$), however, losses would be noticeably lower during normal network operation, e.g. a maximum of 5\%.
...
Using the three-phase current vector, $\textbf{i}_{ss}(t)$ (obtained via substation monitoring, where $(i_{\text{ss},\phi}(t)) = \textbf{i}_{ss}(t)$) a cost, $\zeta_\text{fuse utilisation}(\textbf{i}_{ss}(t))$, is formulated, and defined as follows:
...
Collecting them in $\textbf{i}_{line}(t)$ (where $(i_{\text{line},l,\phi}(t)) = \textbf{i}_\text{line}(t)$) allows the formulation of an extended line utilisation cost, $\zeta_\text{line utilisation}(\textbf{i}_\text{line}(t))$, which is defined as follows:
