...
But in reality internal losses (\hlrem{e.g.}\hladd{for example} conductive losses and magnetic leakage) lead to a dropping voltage level when load increases.
...
This disturbance due to unbalanced phase load may not have an immediate impact, but negative long term effects (\hlrem{e.g.}\hladd{for example} asymmetric load on transformers, rotating machines and increased neutral current) do weaken network assets and cannot be neglected.
...
Here, $\textbf{x}$ can be an arbitrary vector, consisting of scalar values $x_n$ (\hlrem{e.g.}\hladd{for example} $x_n$ may be voltage, current or power measurement per phase $n$).
...
\hlrem{Experts would agree that k}\hladd{K}eeping PF of a system close to unity indicates that it only requires active power to operate which implies that it uses the lowest possible amount of power transmission resources.
...
The negative impact of transmitting \hlrem{power}\hladd{current} through the neutral conductor is further amplified since typical power distribution cables often use neutral conductors with significantly smaller cross-section areas than those used as line conductors.
Results of any such additional \hlrem{power flow}\hladd{current} in the neutral conductor are further deviation of neutral voltages from ground, increased losses and quicker exhaustion of the neutral conductor's power carrying capability (making the system become more prone to failures).
\hladd{A somewhat unconventional approach is used here, since neutral power is estimated instead of neutral current.
...
In comparison to the losses of the \hlrem{entire}\hladd{HV} electricity \hlrem{grid}\hladd{networks}, losses in a LV distribution network are \hlrem{negligibly}\hladd{quantitatively} small.
\hladd{After all, a single LV feeder may distribute up to 500kW of power, whilst HV networks transmit more than 60GW.}
Nonetheless the aggregate effect of reducing those losses could have a noticeable impact on both network efficiency and maximising profit margins (after all, \hlrem{nobody pays}\hladd{energy suppliers cannot charge} for undelivered energy).
...
This equates to 12\% of the total network power, but losses would be noticeably lower during normal network operation, \hlrem{e.g.}\hladd{for instance at} a maximum \hladd{of }\hlrem{around} 5\%.
...
Generally as the distance to the substation increases fewer ``down stream'' customers are connected to a radially expanding feeder and therefore cables can be scaled down (\hlrem{e.g.}\hladd{for example} to save cost).
