...
\nomenclature[I]{$\zeta_u(x)$}{Sample utility function that should be maximised in order to achieve optimality (Chapter \ref{ch1})}
\nomenclature[I]{$\zeta_c(x)$}{Sample cost function that should be minimised in order to achieve optimality (Chapter \ref{ch1})}
...
\nomenclature[I]{$t$}{Time-steps of the simulation, where $t \in \mathbb{Z}^{\geq0}$ (Chapter \ref{ch1})}
\nomenclature[I]{$v_{\text{ss},\phi}$}{Phase voltage at substation for phase $\phi$ at time $t$, where $v_{\text{ss},\phi}(t) \in \textbf{v}_{ss}(t)$ (Chapter \ref{ch1})}
\nomenclature[I]{$\textbf{v}_{ss}(t)$}{Phase voltage vector at time $t$ (Chapter \ref{ch1})}
\nomenclature[I]{$\phi$}{Phase number, typically $\phi \in [1, \dots, \Phi]$ (Chapter \ref{ch1})}
\nomenclature[I]{$\Phi$}{Number of phases, here $\Phi = 3$ (Chapter \ref{ch1})}
\nomenclature[I]{$\zeta_\text{voltage}(\textbf{v}(t))$}{Voltage deviation cost for voltage vector $\textbf{v}$ at time $t$ (Chapter \ref{ch1})}
\nomenclature[I]{$V_\text{ss}$}{Nominal substation voltage (Chapter \ref{ch1})}
\nomenclature[I]{$V_h$}{High-voltage threshold of statutory voltage band (Chapter \ref{ch1})}
\nomenclature[I]{$V_l$}{Low-voltage threshold of statutory voltage band (Chapter \ref{ch1})}
...
\nomenclature[I]{$v_{\text{ESMU},\phi}(t)$}{Phase voltage at ESMU for phase $\phi$ at time $t$, where $v_{\text{ESMU},\phi}(t) \in \textbf{v}_{ESMU}(t)$ (Chapter \ref{ch1})}
...
\nomenclature[I]{$v_{\text{load},i,\phi}(t)$}{Phase voltage of load $i$ for phase $\phi$ at time $t$, where $v_{\text{load},i,\phi}(t) \in \textbf{v}_{load}(t)$ (Chapter \ref{ch1})}
\nomenclature[I]{$\zeta_\text{load voltage}(\textbf{v}(t))$}{Voltage deviation cost for load voltage vector $\textbf{v}$ at time $t$ (Chapter \ref{ch1})}
...
\nomenclature[I]{$\textbf{s}_{ss}(t)$}{Apparent multi-phase power at substation level at time $t$ (Chapter \ref{ch1})}
\nomenclature[I]{$s_{\text{ss},\phi}(t)$}{Apparent single-phase power at substation level for phase $\phi$ at time $t$, where $s_{\text{ss},\phi}(t) \in \textbf{s}_{ss}(t)$ (Chapter \ref{ch1})}
\nomenclature[I]{$\text{UF}(\textbf{x})$}{Function calculating the Unbalance Factor (UF) for any multidimensional vector $\textbf{x}$, where $x_n \in \textbf{x}$ and $n \in \mathbb{Z}^{>0}$ (Chapter \ref{ch1})}
...
\nomenclature[I]{$\zeta_\text{unbalance}(\textbf{s}(t))$}{Power unbalance for multi-phase apparent power vector $\textbf{s}$ at time $t$ (Chapter \ref{ch1})}
...
\nomenclature[I]{$\zeta_\text{PF}(\textbf{s}(t))$}{Power Factor (PF) cost function for multi-phase apparent power vector $\textbf{s}$ at time $t$ (Chapter \ref{ch1})}
...
\nomenclature[I]{$\zeta_\text{neutral load}(\textbf{s}(t))$}{Neutral load cost function for multi-phase apparent power vector $\textbf{s}$ at time $t$ (Chapter \ref{ch1})}
...
\nomenclature[I]{$I_\text{fuse}$}{Nominal fuse rating at substation (Chapter \ref{ch1})}
\nomenclature[I]{$i_{\text{ss},\phi}(t)$}{Single-phase substation current for phase $\phi$ at time $t$ (Chapter \ref{ch1})}
\nomenclature[I]{$\textbf{i}_{ss}(t)$}{Multi-phase substation current at time $t$ (Chapter \ref{ch1})}
\nomenclature[I]{$\zeta_\text{fuse utilisation}(\textbf{i}_{ss}(t))$}{Fuse utilisation cost, derived from multi-phase substation current vector $\textbf{i}_{ss}$ at time $t$ (Chapter \ref{ch1})}
...
\nomenclature[I]{$i_{\text{line},l,\phi}(t)$}{Single-phase line current for phase $\phi$ of line $l$ at time $t$ (Chapter \ref{ch1})}
\nomenclature[I]{$\textbf{i}_\text{line}(t)$}{Multi-phase line currents at time $t$, where $i_{\text{line},l,\phi}(t) \in \textbf{i}_\text{line}(t)$ (Chapter \ref{ch1})}
\nomenclature[I]{$I_{\text{nom},l}$}{Nominal line current for line $l$ (Chapter \ref{ch1})}
...
\nomenclature[I]{$\zeta_\text{losses}(s(t))$}{Losses based cost function (Chapter \ref{ch1})}
\nomenclature[I]{$s_\text{losses}(t)$}{Total apparent power losses in the network (Chapter \ref{ch1})}
