...
Here the difference between highest and lowest \hladd{bus }voltage \hlrem{in}\hladd{of} the network\hladd{is indicated by two lines of the same color. It is this difference that} is noticeably reduced \hlrem{at all times }and their average voltage is brought closer to a nominal voltage level.
...
Whilst the choice of a static power factor for all loads in the \hlrem{\textit{normal}}\hladd{\textit{base}} case resulted in a constant power throughout the day, half-hourly ESMU intervention in the \hlrem{\textit{base}}\hladd{\textit{normal}} case results in a noticeable power factor variation.
...
In fact, \hladd{when compared to the \textit{base} case, the \textit{normal} case lead to a daily energy saving of 0.07kWh.
The \textit{minimised} case however reduced daily losses by 0.36kWh.
This equates to more than five times the}\hlrem{ an additional 6.42\%} energy saving \hladd{that may be}\hlrem{is} achieved simply by adjusting the ESMU's power injection and absorption behaviour\hlrem{ when comparing the \textit{base} and \textit{minimisation} cases}.
...
Whilst the \textit{normal} case noticeably lowered some of the daily demand, power spikes after \hlrem{e.g.}\hladd{for example} 9pm were not addressed at all.
...
\hladd{On each of these box-plots, the central red mark indicates the median, and the bottom and top edges of the blue box indicate the 25$^\text{th}$ and 75$^\text{th}$ percentiles, respectively.
...
In Figure~}\ref{ch1:fig:boxplot-overall-improvements}\hladd{ an individual axis, scaled at 10:1, is used for indicating the cost of neutral power to better visualise its distribution.}
...
In this case the cost reduction (i.e. $-\delta\zeta(\boldsymbol{\alpha})$) is the change in cost from the \textit{normal} ESMU operation case to \textit{minimisation} operation cases, \hlrem{e.g.}\hladd{like} for ``phase unbalance'' $\delta\zeta(\boldsymbol{\alpha}) = \zeta(\alpha_1 = 1) - \zeta(\alpha_6 = 1)$).
...
The underlying data is conditioned in order to meet all prerequisites that are necessary to perform the null hypothesis test, \hlrem{e.g.}\hladd{like} the standard $t$-test.
...
In combination with the preceding table one can therefore determine that \hlrem{e.g. }the impact of optimising operation based on maximum voltage deviation has little to no significant impact on improvements in power factor whilst adjusting ESMU powers to improve the network's power factor has the most significant statistical impact on the chosen key network parameters.
