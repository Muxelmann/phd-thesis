...
In order to treat each cost-function separately $\boldsymbol{\alpha}$ is set to focus on each cost independently, \hlrem{e.g.}\hladd{for example} by setting $\alpha_1 = 1 \text{ and } \alpha_2 = \alpha_3 = \dots = \alpha_9 = 0$.
For simplicity the flowchart in Figure~\ref{ch1:fig:method-evaluation-flowchart} abbreviates the specific cost choice by only indicating which entry in the $\boldsymbol{\alpha}$ vector is set to $1$, \hlrem{e.g.}\hladd{for example} $\zeta(\alpha_1=1)$.
...
\hladd{The above cost function is however limited to target each individual cost separately since $\boldsymbol{\alpha}$ is a binary choice vector.
As discussed in the future work in Chapter~}\ref{ch-conclusions:sec:research-limits-and-future-work}\hladd{ a multi-objective problem may be considered as a next step where costs are weighted against each other.
