...
Here,\hladd{ $t$ is the same positive integer as before to indicate time in minutes,} $\Delta t$ is the sub-half-hourly sampling period of the simulation\hladd{ at minutely time intervals (i.e. 1 minute in this case)} and $K$ is the \hlrem{duration}\hladd{number} of the half-hourly time-slot\hlrem{, i.e.} \hladd{(or }number of sub-half-hourly periods\hladd{)} within the half-hourly slot\hladd{ (i.e. 30 slots per minute in this case)}.
\hladd{It is worth mentioning that the minutely sampling period $\Delta t$ currently equates to one, yet for flexibility of this equation (for instance when using seconds as a base unit) it remains included.}
