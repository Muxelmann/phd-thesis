...
%    % Place nodes
%    \node [cloud] (power_esmu) {$\textbf{s}_{ESMU}(t)$};
%    \node [result, right of=power_esmu, node distance=3cm] (is_charging) {charging\footnotemark[2]};
%    
%    \node [state, below of=is_charging] (charging) {$\times\eta$};
%    \node [state, right of=charging] (discharging) {$\times\frac{1}{\eta}$};
%    \node [cloud, below of=charging] (power_battery) {$s_{battery}(t)$};
%    \node [state, below of=power_battery] (sample) {$\times t_s$};
%    
    % Draw edges
%    \draw [->] (power_esmu) to[] node[auto] {} (is_charging);
%    \draw [->] (is_charging) to[] node[near start] {yes} (charging);
%    \draw [->] (is_charging) to[] node[near start] {no} (discharging);
%    \draw [->] (charging) -- (power_battery);
%    \draw [->] (discharging) -- (power_battery);
%    \draw [->] (power_battery) -- (sample);
...
%    \node [result, below of=evaluate] (decide) {is best candidate better?};
...
\footnotetext[2]{In the flowchart ``charging'' implies that $\sum_{p=1}^{P}s_{ESMU,p}(t) \geq 0$ as explained in Equation \ref{ch1:equ:battery-power-definition}.}