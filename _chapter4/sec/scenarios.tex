...
An AIMD+ scenario in which EV uptake is 100\% and each household has a battery energy storage device.
Here each battery was controlled using the AIMD+ algorithm using the \hlrem{optimised }reference voltage profile.
\end{enumerate}
...
In the following sections, all subscripts of the metrics notate which scenario they represent (\hlrem{e.g.}\hladd{for example} $\zeta_\textbf{C}^{**}$ represents results from \ref{ch4:case-c} and $\zeta_\textbf{D}^{*}$ represents results from \ref{ch4:case-d}).
...
\hladd{It should be noted, that maximised asset utilisation may be of benefit to the DNO since such a state would reflect a best return for the investment.
...
Nonetheless, different usage and ownership scenarios are discussed in Section}~\ref{ch-conclusions:sec:research-limits-and-future-work}\hladd{ of this thesis.}
