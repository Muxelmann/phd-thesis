...
Furthermore, JADE is an application independent package that has become quite popular as seen by the increasing number of publications (\hlrem{e.g.}\hladd{for example}\cite{Karfopoulos2013, Eddy2011, Kuo2013, Mocci2014, Li2017}).
...
More specifically, 10000 repetitions of the scheduling algorithm are queued for a single base\hladd{line} load, but with different $\alpha$ and $\beta$ parameters.
...
\hladd{The Irish dataset }\cite{IrishData2002}\hladd{ comprising several weeks of uninterrupted power data is used since it also provides 1540 different half-hourly load profiles.}
Given th\hlrem{e}\hladd{is large number of} \hlrem{1540 }load profiles\hladd{, provided } from the Irish dataset\hladd{,} and assuming a typical distribution network of 55 customers, a theoretical limit of $6.11\times10^{101}$ simulations would exist.
...
Instead the rate of convergence is determined by an exponential decay function (\hlrem{e.g.}\hladd{for example} similar to Laplace).
