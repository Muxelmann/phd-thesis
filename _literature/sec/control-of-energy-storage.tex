...
Early approaches\hlrem{, e.g.} by Oudalov et al. \cite{Oudalov2007}, who used dynamic programming to generate BESS schedules had relatively high forecast errors due to the inherent difficulty of predicting future loads.
These errors ultimately limit the ability of any given BESS schedule to \hlrem{e.g. }effectively reduce peaks.
...
SPC achieves this behaviour by measuring some properties of the power system (\hlrem{e.g.}\hladd{for example} voltage level or frequency) and comparing those values to an internal target value, i.e. the set-point.
...
However, their solution \hladd{relied on PV and gas turbines to provide active power injection and could} only injected reactive power into the system \hladd{when these are not available - a comparable scenario may occur when BESS completely discharges}\hlrem{, yet BESS in LV distribution networks needs to provide active power from its limited energy resource}.
\hladd{Therefore, voltage compensation may still be realised, but frequency remains unchanged.}
...
This addition produced significantly better results and they shaved daily peaks by around 300kW\hladd{ (from 1.6MW)} in the medium-voltage network.
...
\begin{enumerate}
	\item The \textit{managed system} itself, \hlrem{i.e.}\hladd{like a} power network, that needs to be controlled;
	...
	\item a \textit{decision making system} that uses the provided data to generate certain aims, \hlrem{e.g. }to improve the system state; and
...
The UK \hlrem{power network}\hladd{transmission system} is also a centrally managed system that has seen an increase in complexity, yet \hladd{in 2017} ``\textit{the network is 99.9999\% reliable - a statistic we're proud of}``\cite{NationalGrid2017}.
...
However, this approach highly relies on ICT, as do similar DSM approaches that use \hlrem{e.g.}\hladd{for example} time-of-use tariffs \cite{Deilami2011, Surles2012} or other pricing signals \cite{Masoum2015}.
...
Using traditional SPC on multiple identically-configured DERs can provide an optimal operation conditions, if each DER's control parameters (\hlrem{e.g., }bus voltage) were shared \cite{Thieblemont2017a}.
But in a communication-less environment this requirement cannot be satisfied which is why DER control algorithms have to be improved to prevent \hlrem{e.g.}\hladd{for example} devices located furthest from the substation from being used more frequently than others.
